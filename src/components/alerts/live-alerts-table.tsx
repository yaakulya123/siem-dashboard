...
export function LiveAlertsTable({ alerts }: LiveAlertsTableProps) {
  ...
  useEffect(() => {
    const formatted = alerts.map((a) => ({
      id: 'LIVE-' + Math.random().toString(36).substring(2, 7).toUpperCase(),
      severity: mapSeverity(a.severity),
      title: a.alert_description,
      timestamp: new Date(a.time).toLocaleString(),
      host: a.host_name ?? 'N/A',
      agent: a.agent_name,
      rule: a.rule_groups,
      status: 'open' as 'open',
    }))
      ...
      <div className="overflow-x-auto bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-100 dark:border-gray-700/50 shadow-md">
        <table className="min-w-full divide-y divide-gray-200/70 dark:divide-gray-700/30">
          <thead className="bg-gray-50/80 dark:bg-gray-900/50">
            <tr>
              <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Severity</th>
              <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Alert</th>
              <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Time</th>
              <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Host/Agent</th>
              <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Rule</th>
            ...
            {mappedAlerts.map((alert) => (
              <tr key={alert.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-colors duration-150">
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={clsx('inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium capitalize', getSeverityColor(alert.severity))}>
                    {alert.severity}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="text-sm text-gray-900 dark:text-white font-medium">{alert.title}</div>
                  {/* <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">ID: {alert.id}</div> */}
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{alert.rule}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={clsx('inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium capitalize', getStatusColor(alert.status))}>
                    {alert.status}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
...
// export function LiveAlertsTable({ alert }: LiveAlertsTableProps) {
...
//       title: alert.alert_description,
//       timestamp: new Date(alert.time).toLocaleString(),
//       host: alert.host_name,
//       agent: alert.agent_name,
//       rule: alert.rule_groups,
//       status: 'open'
//     }
